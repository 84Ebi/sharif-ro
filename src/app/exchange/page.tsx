...
interface ExchangePageProps {
  initialTab?: 'buy' | 'sell' | 'history'
}

export default function ExchangePage({ initialTab = 'buy' }: ExchangePageProps) {
  const { user, loading: authLoading } = useAuth()
  const router = useRouter()
  const { t } = useI18n()
  const { showNotification } = useNotification()
  
  const [activeTab, setActiveTab] = useState<'buy' | 'sell' | 'history'>(initialTab)
    ...
    try {
      ...
      if (data.listings) {
        ...
        if (user) {
          ...
          // Fetch my purchases
          // We need a new API endpoint or filter for this. 
          // Since we don't have a dedicated endpoint for "purchased by me", we might need to filter client side if we fetch all, 
          // OR update the API to support filtering by buyerId.
          // Let's assume we update the API to support buyerId.
        ...
        {initialTab !== 'history' ? (
          ...
          <div className="mb-6">
            ...
            <div className="flex bg-white rounded-xl p-1 shadow-sm">
              ...
              <button
                onClick={() => setActiveTab('sell')} // Reusing sell tab logic for "My Listings" view in history page? Or should we make a new tab?
                // Actually, the user said "show all those informations in home page of exchange page" (referring to the new page).
                // Let's make this page show both Purchases and Sales.
                // For now, let's keep it simple. If initialTab is history, we show history.
                // But wait, "My Listings" are currently in the "Sell" tab.
                // Let's create a local state for the sub-tab in history view.
                className={`flex-1 py-2 rounded-lg font-semibold transition-all ${
                  activeTab === 'sell' 
                    ? 'bg-green-600 text-white shadow-md' 
                    : 'text-gray-600 hover:bg-gray-50'
